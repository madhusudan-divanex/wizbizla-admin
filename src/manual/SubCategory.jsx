...
function SubCategory() {
    ...
    const fetchSubCategory = async (pageNumber = page || 1, searchQuery = search) => {
        try {
            const result = await getSecureApiData(`get-subcategory?page=${pageNumber}`)
            if (result.status) {
                setSubCategoryList(result.categoryData)
                setPage(result.currentPage);
                setPages(result.totalPages);
                setTotal(result.totalSubCategory);
            } else {
                toast.error(result.message)
            }
            // const res = await axios.get(`https://api.wizbizlaonboard.com/api/categorys/all?page=${pageNumber}&q=${searchQuery}`);
            // if (res.data.success) {
            //     setSubCategoryList(res.data.categorys || []);
    ...
    const handleDelete = (id) => {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                try {
                    const result = await deleteApiData(`delete-subcategory/${id}`)
                    if (result.status) {
                        ...
                        <div className="container mt-4">
                            <div className="card">
                                <div className="card-header">
                                    <h5 className="mb-0">Sub Category Data</h5>
                                </div>
