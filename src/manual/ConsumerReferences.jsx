...
const ConsumerReferences = () => {
    ...
    const fetchUsers = async (pageNumber = page, searchQuery = search) => {
        try {
            const result = await getSecureApiData(
                `consumer-reference?page=${pageNumber}`
            );
    ...
    const updateStatus = async (refId,  status) => {
        const data = { refId,  status }
        try {
            const result = await postApiData(
                `consumer-reference-action`, data
            );
                ...
                <div className='main-content'>
                    <div className="row mb-3" >
                        ...
                        <div className="col-sm-6">
                            ...
                            <input id='name' type="text" className="form-control" value={refData?.userId?.email} disabled />
                        </div>
                        ...
                        <div className="row mb-3">
                            <div className="col-sm-6">
                                ...
                                <input id='name' type="text" className="form-control" value={refData?.name} disabled />
                            </div>
                            <div className="col-sm-6">
                                <label htmlFor='name'>Contact Detail</label>
                                <input id='name' type="text" className="form-control" value={refData?.contact} disabled />
                            </div>
                            <div className="col-sm-6">
                                <label htmlFor='name'>Work Together</label>
                                <input id='name' type="text" className="form-control" value={refData?.workTogether} disabled />
                            </div>
                            <div className="col-sm-6">
                                <label htmlFor='name'>Relationship</label>
                                <input id='name' type="text" className="form-control" value={refData?.relationship} disabled />
                            </div>
                            <div className="col-sm-6">
                                <label htmlFor='name'>Status</label>
                                <select id='name' name='status' type="text" className="form-select" value={refData.status}
                                    onChange={(e) => {
                                        const newStatus = e.target.value;
                                        setRefData({ ...refData, status: newStatus });
                                    }}>
                            ...
                            <div className='d-flex my-3 justify-content-between'>
                                ...
                                <button onClick={() => updateStatus(refData?._id,  refData?.status)} className='btn btn-success'>Update</button>
                                <button onClick={() => setIsScam(false)} className='btn btn-secondary'>Back</button>
                            </div>
                        </div>
                </div>
                    ...
                    <div className="container-fluid">
                        <div className="card">
                            <div className="card-header">
                                <h5 className="mb-0">Consumer References</h5>
                                                ...
                                                userList?.map((cat, index) => {
                                                    return (
                                                        <tr key={cat._id}>
                                                            ...
                                                            <td>{cat?.name}</td>
                                                            <td>{cat?.status}</td>
                                                            <td>{new Date(cat?.createdAt)?.toLocaleDateString('en-GB', {
                                                                day: '2-digit',
                                                                month: '2-digit',
                                                                year: 'numeric'
                                                            })}</td>
...
export default ConsumerReferences;
