    ...
    const fetchUsers = async (pageNumber = page, searchQuery = search) => {
        try {
            const result = await getSecureApiData(
                `get-all-users?page=${pageNumber}&search=${encodeURIComponent(searchQuery)}&type=provider&status=${!data?'pending':data}`
            );
                                                    ...
                                                    <tr key={cat._id}>
                                                        ...
                                                        <td className="text-end">
                                                            <div className="d-flex justify-content-end gap-2">
                                                                <Link to={cat?.onBoarding ? `/user/view/${cat._id}` : `/user/detail/${cat._id}`} className="btn btn-sm btn-light"><FiEye /></Link>
