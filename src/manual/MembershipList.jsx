...
function MembershipList() {
    ...
    const fetchMembership = async (pageNumber = page, searchQuery = search) => {
        try {
            const result=await getSecureApiData('get-membership')
            if(result.status){
                setMembershipList(result.membershipData)
            }else{
                toast.error(result.message)
            }
            // const res = await axios.get(`https://api.wizbizlaonboard.com/api/memberships/all?page=${pageNumber}&q=${searchQuery}`);
            // if (res.data.success) {
            //     setMembershipList(res.data.memberships || []);
            //     setPage(res.data.currentPage);
            //     setPages(res.data.totalPages);
            //     setTotal(res.data.totalMemberships);
            // }
        } catch (error) {
            console.log("error fetch memberships", error);
        }
    }
