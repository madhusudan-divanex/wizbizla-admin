...
function ScamType() {
    ...
    const [scamTypeList, setScamTypeList] = useState([]);
    const [page, setPage] = useState(1);
    const [pages, setPages] = useState(1);
    const [total, setTotal] = useState(0);
    const [search, setSearch] = useState('');

    const fetchScamType = async (pageNumber = page || 1, searchQuery = search) => {
        try {
            const result = await getSecureApiData(`scam-type?page=${pageNumber}`)
            if (result.status) {
                setScamTypeList(result.categoryData)
                setPage(result.currentPage);
                setPages(result.totalPages);
                setTotal(result.totalScamType);
            } else {
                toast.error(result.message)
            }
            // const res = await axios.get(`https://api.wizbizlaonboard.com/api/categorys/all?page=${pageNumber}&q=${searchQuery}`);
            // if (res.data.success) {
            //     setScamTypeList(res.data.categorys || []);
    ...
    const handleSubmit = async (e) => {
        ...
        if (isEdit) {
            ...
            try {
                const result = await updateApiData('scam-type', data)
                if (result.status) {
            ...
            try {
                const result = await postApiData('scam-type', {name})
                if (result.status) {
                    toast.success("Scam Type created successfully");
                   setIsEdit(false)
                   fetchScamType()
                    setIsSub(false)
                } else {
    ...
    const handleDelete = (id) => {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                try {
                    const result = await deleteApiData(`scam-type/${id}`)
                    if (result.status) {
                        ...
                        <div className="container mt-4">
                            <div className="card">
                                <div className="card-header">
                                    <h5 className="mb-0">Scam Type Data</h5>
                                </div>
                ...
                : <div className='main-content'>
                    <div className='row'>
                        <div className="container-fluid">
                            <div className="card">
                                <div className="card-header">
                                    <h5 className="mb-0">Scam Type</h5>
                                    <div>
                                ...
                                <div className="card-body table-responsive">
                                    <table className="table table-hover table-bordered">
                                        ...
                                        <tbody>
                                            {
                                                scamTypeList?.length > 0 ? (
                                                    scamTypeList?.map((item, index) => {
                                                        return (
                                                            <tr key={item._id}>
                                                    ...
                                                    <tr>
                                                        <td colSpan={6} className='text-center'>No Scam Type Found</td>
                                                    </tr>
                                                )
                                            }
                                        </tbody>
